...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const getFolderCount = (folderName: string) => {
    return notes.filter((n) => n.folder && n.folder.toLowerCase() === folderName.toLowerCase())
      .length;
  };

  const getTagCount = (tagName: string) => {
    return notes.filter((n) =>
      Array.isArray(n.tags) && n.tags.map((t) => t.toLowerCase()).includes(tagName.toLowerCase())
    ).length;
  };
    ...
    >
      ...
      <div className="p-4 flex items-center justify-between border-b border-neutral-100 dark:border-neutral-800/80">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="w-8 h-8 rounded-lg bg-amber-400 text-neutral-950 flex items-center justify-center font-bold tracking-tight shadow-xs shrink-0">
            M
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-4 text-xs custom-scrollbar">
        ...
        <div id="sidebar-section-overview" className="border-b border-neutral-100 dark:border-neutral-800/40 pb-3">
          <button
            ...
            <div className="mt-1 space-y-0.5">
              ...
              >
                <div className="flex items-center gap-2.5">
                  <Star
                    className={`w-4 h-4 ${
                      filter.onlyPinned ? 'fill-amber-400 text-amber-500 dark:text-amber-400' : 'text-neutral-400'
                    }`}
                  />
                  <span>Pinned</span>
                </div>
        ...
        <div id="sidebar-section-folders" className="border-b border-neutral-100 dark:border-neutral-800/40 pb-3">
          ...
          {isFoldersOpen && (
            <div className="mt-1 space-y-1">
              {showAddFolder && (
                <form onSubmit={handleAddFolderSubmit} className="px-1 py-1 mb-1.5">
                  <div className="flex items-center gap-1.5">
                    <input
                      id="new-folder-input"
                      type="text"
              ...
              <div className="space-y-0.5">
                {allFolders.map((folder) => {
                  const isSelected =
                    filter.folder.toLowerCase() === folder.toLowerCase() && !filter.onlyPinned;
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {isSelected ? (
                          <FolderOpen className={`w-4 h-4 shrink-0 ${folderStyle.iconClass}`} />
                        ) : (
                          <Folder className={`w-4 h-4 shrink-0 ${folderStyle.iconClass}`} />
                        )}
            ...
            <div className="mt-1.5">
              {allTags.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-1.5 px-1 pt-0.5">
                  {allTags.map((tag) => {
                    const isSelected = filter.tag?.toLowerCase() === tag.toLowerCase();
      ...
      <div className="p-3 border-t border-neutral-200 dark:border-neutral-800/80 bg-neutral-50/70 dark:bg-neutral-950/40 shrink-0">
        ...
        >
          <div className="flex items-center gap-2 pl-2">
            ...
            <span className="text-[11px] font-medium text-neutral-700 dark:text-neutral-300">Theme</span>
          </div>
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <div className="text-xs font-medium text-neutral-800 dark:text-white truncate">
                {user?.email ? user.email : user?.isAnonymous ? 'Guest User' : 'Local Workspace'}
              </div>
              <div className="text-[10px] text-neutral-500 dark:text-neutral-400 truncate">
                {user ? (user.isAnonymous ? 'Temporary session' : 'Cloud synchronized') : 'Sign in to sync cloud'}
              </div>
            </div>
          </div>
